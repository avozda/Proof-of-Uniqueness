    ...
    H: Hasher,
{
    ...
    /// Insert key-leaf entry into the 254-bit SMT. Returns a new root hash.
    pub fn insert(&mut self, root: Option<&Hash>, key: &Hash, leaf: &Hash) -> Result<Option<Hash>> {
        // Check if key fits within our 254-bit depth limit
        let key_bits = Bits::new(key);
        if key_bits.len() > SMT_DEPTH {
            return Err(Errors::new(&format!(
                "Key too long: {} bits, maximum is {} bits",
                key_bits.len(),
                SMT_DEPTH
            )));
        }
        ...
        match root {
            None => {
                let (hash, bits) = (leaf, key_bits);
                self.put_node(Node::new(Some(Unit { hash, bits }), None))
            }
            Some(root) => self.put(root, key_bits, leaf),
        }
    }
    ...
    fn put(&mut self, root: &[u8], bits: Bits, leaf: &[u8]) -> Result<Option<Hash>> {
        ...
        if bits.len() > SMT_DEPTH {
            return Err(Errors::new(&format!(
                "Maximum SMT depth {} exceeded",
                SMT_DEPTH
            )));
        }
