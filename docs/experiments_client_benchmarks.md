...
Revocation no longer uses a Noir/Barretenberg proof. It is authorized by an EIP-712 wallet signature and verified directly by the smart contract.
...
3. **Revocation no longer contributes proving latency**
  - The previous revocation circuit was removed in favor of wallet signatures.
...
5. **Proofs stay succinct regardless of circuit size**
  - The active auth and enrollment circuits produce 2,144-byte Barretenberg Ultra proofs; only the number of public inputs differs (4 for auth, 10 for enrollment).
...
```bash
node scripts/benchmark-current-stack.mjs             # auth + micros
